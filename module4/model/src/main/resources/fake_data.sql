...
delete
from user_order_gift_certificates
where true;
        ...
        DO
            ...
            WHILE tQty > 0
                DO
                    ...
                    INSERT IGNORE INTO gift_certificate_tags(tag_id, gift_certificate_id)
                        VALUE (t_id, c_id);
                    SET tQty = tQty - 1;
                END WHILE;
            SET c_id = c_id + 1;
    ...
    WHILE i <= 1000
        DO
            INSERT INTO users(id, login, password, status)
                VALUE (i, CONCAT('username_', i), TO_BASE64('Psw123_'), 'ACTIVE');
            SET i = i + 1;
        END WHILE;
END;
...
BEGIN
    ...
    WHILE i <= 755
        DO
            ...
            INSERT IGNORE INTO user_orders(id, user_id, cost)
        ...
        DO
            ...
            WHILE cQty > 0
                DO
                    ...
                    INSERT IGNORE INTO user_order_gift_certificates(user_order_id, gift_certificate_id)
                        VALUE (uo_id, c_id);
                    SET cQty = cQty - 1;
                END WHILE;
            SET uo_id = uo_id + 1;
